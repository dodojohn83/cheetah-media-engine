...
CheetahPlayerQml::CheetahPlayerQml(QObject *parent) : QObject(parent) {
    if (cheetah_player_create(&player_) != CHEETAH_RESULT_OK) {
        player_ = nullptr;
    } else if (player_ != nullptr) {
        cheetah_player_set_event_callback(player_, &CheetahPlayerQml::eventCallback, this);
    }
}

CheetahPlayerQml::~CheetahPlayerQml() {
    if (player_ != nullptr) {
        cheetah_player_destroy(&player_);
    }
}

bool CheetahPlayerQml::configure(const QString &json) {
    if (player_ == nullptr) {
        return false;
    }
...
bool CheetahPlayerQml::load(const QString &url, bool isLive) {
    if (player_ == nullptr) {
        return false;
    }
...
QString CheetahPlayerQml::state() const {
    if (player_ == nullptr) {
        return QString();
    }
...
QObject *CheetahPlayerQml::videoSurface() const {
    // Placeholder: WP-59 will return the actual rendering surface.
    return const_cast<CheetahPlayerQml *>(this);
}

void CheetahPlayerQml::onEvent(const CheetahEvent *event) {
...
void CheetahPlayerQml::eventCallback(const CheetahPlayer * /*player*/,
                                     const CheetahEvent *event,
                                     void *userdata) {
    auto *obj = static_cast<CheetahPlayerQml *>(userdata);

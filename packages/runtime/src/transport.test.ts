...
describe('FetchTransport', () => {
  ...
  it('rejects non-finite or non-positive transport config', async () => {
    const transport = new FetchTransport({ url: 'https://example.com/stream', timeoutMs: NaN });
    const err = await new Promise<{ code: number }>((resolve, reject) => {
      transport.start(
        () => reject(new Error('unexpected chunk')),
        (error) => resolve(error),
  ...
  it('rejects NaN maxBytes and negative maxRetries', async () => {
    const transport = new FetchTransport({ url: 'https://example.com/stream', maxBytes: NaN, maxRetries: -1 });
    const err = await new Promise<{ code: number }>((resolve, reject) => {
      transport.start(
        () => reject(new Error('unexpected chunk')),
        (error) => resolve(error),
